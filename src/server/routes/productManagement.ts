...
function buildProductRow(input: any, existing: any = {}): any {
  ...
  return {
    ...
    model_or_part: input.model_or_part !== undefined ? (input.model_or_part || null) : (existing.model_or_part ?? null),
    code: input.code !== undefined ? String(input.code).toUpperCase().trim() : existing.code,
    name: input.name !== undefined ? String(input.name).trim() : existing.name,
...
async function syncVariants(productId: string, variations: any[], productCode: string): Promise<any[]> {
  ...
  for (const v of list) {
    ...
    await upsertVariant({
      ...
      sku,
      epurchase_item_code: v.epurchase_item_code !== undefined ? (v.epurchase_item_code || null) : null,
      model_or_part: v.model_or_part !== undefined ? (v.model_or_part || null) : null,
