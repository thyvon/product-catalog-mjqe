...
import { RefreshCw, Trash2, Search, FileText, Download, Upload, Eye, XCircle, CheckCircle, CloudUpload, AlertCircle } from "lucide-react";
...
export default function StockIssueItemsPage() {
  ...
  const fetchItems = async () => {
    ...
    try {
      ...
      if (search) params.set("warehouse", search);
    ...
    reader.onload = (e) => {
      try {
        ...
        const formatted = json.map((row: any) => ({
          ...
          transactionDate: row["Date"] || row["date"] || row["DATE"] || "",
  ...
  const submitImport = async () => {
    ...
    try {
      ...
      setTimeout(() => { setShowImport(false); fetchItems(); }, 1200);
  ...
  const handleDelete = async (id: string) => {
    ...
    try { const res = await fetch(`/api/stock-issue-items/${id}`, { method: "DELETE" }); if (res.ok) fetchItems(); }
  ...
  return (
    <div className="p-4 lg:p-6 space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-lg font-black text-slate-900 dark:text-gray-100 tracking-tight">Stock Issue Items</h1>
          <p className="text-xs text-slate-400 dark:text-gray-500 mt-1">{items.length} item{items.length !== 1 ? "s" : ""}</p>
        </div>
      ...
      {showImport && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="fixed inset-0 bg-black/30" onClick={() => { if (!importLoading) setShowImport(false); }} />
          <div className="relative bg-white dark:bg-gray-900 w-full max-w-4xl rounded-3xl shadow-2xl border border-slate-100 dark:border-gray-800 flex flex-col max-h-[90vh]">
            <div className="p-5 border-b border-slate-100 dark:border-gray-800 flex justify-between items-center bg-slate-50/50 dark:bg-gray-800/50">
              ...
              <button onClick={() => { if (!importLoading) setShowImport(false); }} className="p-1.5 hover:bg-slate-200 dark:hover:bg-gray-800 text-slate-400 rounded-full cursor-pointer transition-all">
            ...
            <div className="p-4 border-t border-slate-100 dark:border-gray-800 flex justify-end gap-3 bg-slate-50/30 dark:bg-gray-800/30">
              <button onClick={() => setShowImport(false)} disabled={importLoading}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100 dark:border-gray-800 bg-slate-50/50 dark:bg-gray-800/30">
                <th className="text-left px-2 py-3 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Date</th>
                <th className="text-left px-2 py-3 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Code</th>
                <th className="text-left px-2 py-3 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Description</th>
                <th className="text-right px-2 py-3 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Qty</th>
                <th className="text-left px-2 py-3 text-[10px] font-bold text-slate-400 uppercase tracking-widest">UoM</th>
                <th className="text-right px-2 py-3 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Unit Price</th>
                <th className="text-right px-2 py-3 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Total Amount</th>
                <th className="text-left px-2 py-3 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Requester</th>
                <th className="text-left px-2 py-3 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Campus</th>
                <th className="text-left px-2 py-3 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Division</th>
                <th className="text-left px-2 py-3 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Department</th>
                <th className="text-left px-2 py-3 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Description/Purpose</th>
                <th className="text-left px-2 py-3 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Ref.No</th>
                <th className="text-left px-2 py-3 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Trans Type</th>
                <th className="text-left px-2 py-3 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Account Code</th>
                <th className="text-center px-2 py-3 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Actions</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                Array.from({ length: 3 }).map((_, i) => (
                  <tr key={i} className="border-b border-slate-50 dark:border-gray-800/50">
                    {Array.from({ length: 16 }).map((_, j) => <td key={j} className="px-2 py-3"><div className="h-4 bg-slate-200 dark:bg-gray-700 rounded animate-pulse" /></td>)}
                  </tr>
                ))
              ) : items.length === 0 ? (
                <tr><td colSpan={16} className="px-4 py-12 text-center">
                  <FileText className="w-8 h-8 text-slate-300 dark:text-gray-600 mx-auto mb-2" />
                  <p className="text-xs text-slate-400 dark:text-gray-500">No items imported yet.</p>
                  <button onClick={() => setShowImport(true)} className="mt-3 text-xs font-bold text-indigo-600 cursor-pointer">Import from Excel</button>
                </td></tr>
              ) : (
                items.map((item) => (
                  <tr key={item.id} className="border-b border-slate-50 dark:border-gray-800/50 hover:bg-slate-50/50 dark:hover:bg-gray-800/20">
                    <td className="px-2 py-3 text-[10px] text-slate-500 font-mono whitespace-nowrap">
                      {item.transactionDate
                        ? new Date(item.transactionDate).toLocaleDateString("en-US", { month: "short", day: "2-digit", year: "numeric" })
                        : ""}
                    </td>
                    <td className="px-2 py-3 text-xs font-mono font-bold text-slate-700 dark:text-gray-300 whitespace-nowrap">{item.itemCode}</td>
                    <td className="px-2 py-3 text-xs text-slate-600 dark:text-gray-400 max-w-[160px] truncate">{item.description}</td>
                    <td className="px-2 py-3 text-xs text-right font-mono text-slate-700 dark:text-gray-300 whitespace-nowrap">{item.quantity}</td>
                    <td className="px-2 py-3 text-xs text-slate-500">{item.uom}</td>
                    <td className="px-2 py-3 text-xs text-right font-mono text-slate-700 dark:text-gray-300 whitespace-nowrap">${Number(item.unitPrice).toFixed(2)}</td>
                    <td className="px-2 py-3 text-xs text-right font-mono text-slate-700 dark:text-gray-300 whitespace-nowrap">${Number(item.totalPrice).toFixed(2)}</td>
                    <td className="px-2 py-3 text-xs text-slate-600 dark:text-gray-400">{item.requesterName}</td>
                    <td className="px-2 py-3 text-xs text-slate-600 dark:text-gray-400">{item.campus}</td>
                    <td className="px-2 py-3 text-xs text-slate-600 dark:text-gray-400">{item.division}</td>
                    <td className="px-2 py-3 text-xs text-slate-600 dark:text-gray-400">{item.department}</td>
                    <td className="px-2 py-3 text-xs text-slate-500 max-w-[120px] truncate">{item.remarks}</td>
                    <td className="px-2 py-3 text-xs font-mono text-slate-600 dark:text-gray-400 whitespace-nowrap">{item.referenceNo}</td>
                    <td className="px-2 py-3 text-xs text-slate-500">{item.transactionType}</td>
                    <td className="px-2 py-3 text-xs font-mono text-slate-500">{item.accountCode}</td>
